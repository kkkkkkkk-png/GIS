...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        const response = await axios.get("http://localhost:3000/api/liquid")
    ...
    async deleteItem(id) {
      try {
        await axios.delete(`http://localhost:3000/api/liquid/${id}`)
    ...
    async submitForm() {
      try {
        ...
        const url = this.editing
          ? `http://localhost:3000/api/liquid/${this.formData["Data ID"]}`
          : "http://localhost:3000/api/liquid"
